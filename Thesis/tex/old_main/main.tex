...
\delta & Losely used to designate a time delay the length of which will be clear from the context.
...
\scthinkagent{i} & Thinking time of HFT SC agent $i$ where $\scthinkagent{i} \sim \lvert \mathcal{N}(\scthinkmu, \scthinks) \rvert$ \\
...
\stlatency, \stlatencymu{} and \stlatencys & Delay with which the slow trader observes the fundamental: $\stlatency \sim \lvert \mathcal{N}(\stlatencymu, \stlatencymu) \rvert$
