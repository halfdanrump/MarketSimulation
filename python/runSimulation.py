...
if __name__=='__main__':
    ...
    parameters['logFolder'] = '/Users/halfdan/Dropbox/Waseda/Research/MarketSimulation/logs/'
    ...
    for ssmm_nAgents in ssmm_nAgents_range:
        for sc_nAgents in sc_nAgents_range:
            ...
            for rep in xrange(n_reps):
                parameters['rep'] = rep
                parString = ''
                for (par, val) in parameters.items(): parString += '-D%s=%s '%(par,val)
                command = "java -d64 -Xms512m -Xmx4g -DnRounds=%(nRounds)s -DlogFolder=%(log_folder)s%(rep)s/ -DexperimentName=%(experiment_name)s -Dssmm_nAgents=%(ssmm_nAgents)s -Dsc_nAgents=%(sc_nAgents)s -jar %(jar_path)s" % parameters   
                print command
                ### Run simulation
                processes.append(Popen(command.split(' '), stdout=open(devnull)))
            ...
            for rep in xrange(n_reps):                       
                ### Read files        
                ob_round_based = np.genfromtxt(parameters['log_folder'] + '%s/columnLog_roundBased_orderbook(0,0).csv'%rep, names=True, dtype=int, delimiter=',', usecols=(4,5))
                stock_round_based = np.genfromtxt(parameters['log_folder'] + '%s/columnLog_roundBased_stock0.csv'%rep, names=True, dtype=int, delimiter=',', usecols=(0,1))
                trades = np.genfromtxt(parameters['log_folder'] + '%s/columnLog_transactionBased_stock0.csv'%rep, names=True, dtype=int, delimiter=',', usecols=(0,2))
